...
Ballot.prototype.tally = function (cb) {
  if (this.finalized) return process.nextTick(cb, new Error('Ballot must be finalized before it can be tallied'))

  var counts = this.candidates.reduce(function (c, k) {
    c[k] = 0
    return c
  }, {})
