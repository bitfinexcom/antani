...
Ballot.prototype._ready = function (cb) {
  ...
  function open (cb) {
    self.storage.stat(function (_, st) {
      if (!st) return cb()
      self.storage.read(0, Math.min(st.size, 65536), function (_, start) {
        if (!start) return cb()
        const i = start.indexOf(',"votes":[')
        if (i === -1) return cb()
        const header = JSON.parse(start.slice(0, i + 10).toString() + ']}')
  ...
  function write (data, enc, cb) {
    ...
    self.first = false
    self.offset += Buffer.byteLength(JSON.stringify(data)) + 1
    cb(null)
  }
  ...
  function done (err) {
    ...
    self.storage.stat(function (err, st) {
      if (err) return cb(err)
      self.storage.read(st.size - 3, 3, function (err, buf) {
        if (buf && buf.toString() === '}]}') {
          self.finalized = true
        }
        cb(null)
  ...
  self.verifyVote(vote, function (err) {
    ...
    self.ready(function (err) {
      ...
      self.lock(function (release) {
        ...
        if (Buffer.byteLength(preamble) >= 65536) return cb(new Error('Headers bigger than 64KB not supported'))
