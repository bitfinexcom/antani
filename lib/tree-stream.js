...
function createTreeStream () {
  ...
  function flush (cb) {
    ...
    assert(Number.isSafeInteger(root.balance))
  ...
  function write (data, enc, cb) {
    ...
    assert(Number.isSafeInteger(data.balance))
    assert(typeof data.key === 'string')
    assert(data.key.length === crypto.PUBLICKEY_LEN)
    assert(typeof data.secretKey === 'string')
    assert(data.secretKey.length === crypto.SECRETKEY_LEN)
